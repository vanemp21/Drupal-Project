...
(function ($, Drupal) {
  Drupal.behaviors.userList = {
    attach: function (context, settings) {
      var $form = $('#user-list-filter', context);
      var $userListContainer = $('#user-list-container', context);
      var $pagination = $('#user-list-pagination', context);

      function fetchUsers(page) {
        var query = $form.serialize(); // Serializa los datos del formulario

        $.ajax({
          url: '/path-to-your-ajax-endpoint', // Asegúrate de que esta URL sea correcta
          type: 'GET',
          data: $.extend(query, { page: page }), // Añade la página a los datos de búsqueda
          dataType: 'json',
          success: function (data) {
            if (data.usuarios.length > 0) {
              var rows = data.usuarios.map(function (user) {
                return '<tr>' +
                  '<td>' + user.name + '</td>' +
                  '<td>' + user.surname1 + '</td>' +
                  '<td>' + user.email + '</td>' +
              ...
              $userListContainer.find('tbody').html(rows);
            } else {
              $userListContainer.find('tbody').html('<tr><td colspan="3">No results found.</td></tr>');
            }
            updatePagination(page, Math.ceil(data.total / 5)); // Actualiza la paginación
          }
        });
      }
      ...
      function updatePagination(currentPage, totalPages) {
        ...
        for (var i = 1; i <= totalPages; i++) {
          paginationLinks += '<a href="#" data-page="' + i + '">' + i + '</a> ';
        }
      ...
      $form.on('submit', function (e) {
        ...
        fetchUsers(1); // Carga la primera página
      });
