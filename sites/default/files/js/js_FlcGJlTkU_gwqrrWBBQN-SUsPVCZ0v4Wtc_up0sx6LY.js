  ...
  Drupal.behaviors.userList = {
    attach: function (context, settings) {
      ...
      $('#user-search-form', context).submit(function (e) {
        ...
        console.log('Form data:', formData); // Depuración

        $.post('/prueba_drupal/user-list/ajax', formData, function (data) {
          console.log('Data received:', data); // Depuración

          // Verifica si 'usuarios' está definido y es un array
          if (data.usuarios && Array.isArray(data.usuarios)) {
            let html = '<ul>';
            data.usuarios.forEach(function (user) {
              html += `<li>${user.name} ${user.surname1} ${user.surname2} - ${user.email}</li>`;
            });
            html += '</ul>';
            $('#user-list').html(html);
          } else {
            $('#user-list').html('<p>No se encontraron resultados.</p>');
          }
        }).fail(function (jqXHR, textStatus, errorThrown) {
          console.error('Error in AJAX request:', textStatus, errorThrown);
          $('#user-list').html('<p>Error al cargar los datos.</p>');
        });
      });
      ...
      function loadPage(page) {
        ...
        console.log('Form data for page', page, ':', formData); // Depuración

        $.get('/prueba_drupal/user-list/ajax', { page: page, ...$.deparam(formData) }, function (data) {
          console.log('Data received for page', page, ':', data); // Depuración
