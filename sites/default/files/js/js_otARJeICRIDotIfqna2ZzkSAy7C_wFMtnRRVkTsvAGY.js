...
;
(function ($, Drupal) {
  Drupal.behaviors.userListFilter = {
    attach: function (context, settings) {
      // Asegurarnos de que no se adjunten múltiples eventos de cambio.
      $('.user-list-search', context).once('user-list-search-attached').each(function () {
        let inputField = $(this);

        // Eliminar el uso de .once() directamente y utilizar un control de eventos.
        if (!inputField.data('ajax-bound')) {
          inputField.data('ajax-bound', true);

          inputField.on('input', function () {
            let searchValue = inputField.val();

            // Realizar la llamada AJAX para filtrar usuarios
            $.ajax({
              url: '/user-list/ajax',
              type: 'GET',
              data: { name: searchValue },
              success: function (response) {
                // Reemplazar el contenido del listado de usuarios con los datos filtrados.
