...
(function ($, Drupal) {
  Drupal.behaviors.userList = {
    attach: function (context, settings) {
      ...
      $('#user-list-form', context).off('submit').on('submit', function (e) {
        e.preventDefault();
        fetchUsers(1);
      });
      ...
      function fetchUsers(page) {
        $.ajax({
          ...
          success: function (data) {
            let rows = '';
            // Itera sobre los usuarios devueltos por el AJAX.
            $.each(data.usuarios, function (index, user) {
              rows += '<tr>' +
                '<td>' + Drupal.checkPlain(user.name) + '</td>' +
                '<td>' + Drupal.checkPlain(user.surname1) + '</td>' +
                '<td>' + Drupal.checkPlain(user.surname2) + '</td>' +
                '<td>' + Drupal.checkPlain(user.email) + '</td>' +
              '</tr>';
            });
            // Actualiza la tabla de usuarios.
            $('#user-list-table tbody').html(rows);

            // Calcula y construye la paginación.
            let total_pages = Math.ceil(data.total / 5);
            let pagination = '';
            ...
            // Reasigna el evento a los nuevos enlaces de paginación
