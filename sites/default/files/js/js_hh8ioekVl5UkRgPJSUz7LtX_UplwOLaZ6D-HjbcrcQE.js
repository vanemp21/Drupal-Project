...
(function ($, Drupal) {
  Drupal.behaviors.userListFilter = {
    attach: function (context, settings) {
      // Seleccionamos el campo de búsqueda sin usar .once()
      let inputField = $('.user-list-search', context);

      // Verificamos si el evento ya ha sido ligado, para evitar múltiples adjuntos
      if (!inputField.data('event-attached')) {
        inputField.data('event-attached', true);

        // Adjuntamos el evento 'input'
